...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            elif loss_type == 'ergas_loss':
                from networks.BDPN import ergas_loss
                self.criterion_pix = ergas_loss()
            else:
    ...
    def _net_init(self, init_type='kaiming'):
        ...
        # init_weights(self.model, init_type, nonlinearity='relu')
    ...
    def train_step(self):
        ...
        for i in range(self.split_batch):
            ...
            loss_sbatch = loss_sbatch/self.split_batch
...
#        for aug in lr_list:
    ...
    def get_current_visual(self, need_np=True, need_HR=True):
        ...
        if need_np:  out_dict['LR'], out_dict['SR'] = util.pan_Tensor2np([out_dict['LR'], out_dict['SR']],
                                                                        self.opt['run_range'], self.opt['img_range'])
        if need_HR:
    ...
    def save_current_log(self):
        ...
        self.records['CC'].append(self.records['CC'][self.best_epoch - 1])
        self.records['RMSE'].append(self.records['RMSE'][self.best_epoch - 1])
