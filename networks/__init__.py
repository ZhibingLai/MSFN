...
def weights_init_kaiming(m, scale=1, nonlinearity='leaky_relu'):
    classname = m.__class__.__name__
    if isinstance(m, (nn.Conv2d, nn.ConvTranspose2d)):
        if classname != "MeanShift":
            print('initializing [%s] ...' % classname)
...
def init_weights(net, init_type='kaiming', scale=1, std=0.02, nonlinearity='leaky_relu'):
    ...
    if init_type == 'normal':
        ...
        weights_init_kaiming_ = functools.partial(weights_init_kaiming, scale=scale, nonlinearity=nonlinearity)
