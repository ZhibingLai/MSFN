...
#    opt = option.parse(parser.parse_args().opt)
    opt = option.parse('./options/test/test_SRFBN_example.json')
    ...
    for bm, test_loader in zip(bm_names, test_loaders):
        ...
        for iter, batch in enumerate(test_loader):
            ...
            if need_HR:
                psnr, ssim = util.pan_calc_metrics(visuals['SR'], visuals['HR'], crop_border=scale,
        ...
        if need_HR:
            ...
            print("PSNR: %.4f      SSIM: %.4f      Speed: %.4f" % (sum(total_psnr)/len(total_psnr),
                                                                  sum(total_ssim)/len(total_ssim),
                                                                  sum(total_time[1:])/(len(total_time)-1)))
        else:
